...
async function testSellerAuth() {
  ...
  try {
    ...
    console.log('📊 Seller Request Response Headers:', Object.fromEntries(sellerRequestResponse.headers))
    ...
    if (sellerRequestResponse.ok) {
      ...
      // Let's test if middleware is working by checking headers
      console.log('🔍 Debugging: Testing different header formats...')
      ...
      for (let i = 0; i < testHeaders.length; i++) {
        console.log(`🧪 Testing header format ${i + 1}:`, testHeaders[i])
        
        const testResponse = await fetch(`${baseURL}/seller-requests`, {
          method: 'POST',
          headers: {
            ...testHeaders[i],
            'Content-Type': 'application/json'
        ...
        console.log(`📊 Response ${i + 1}: Status ${testResponse.status}`)
        if (!testResponse.ok) {
          console.log(`📊 Error ${i + 1}:`, await testResponse.text())
        }
      }
    }
