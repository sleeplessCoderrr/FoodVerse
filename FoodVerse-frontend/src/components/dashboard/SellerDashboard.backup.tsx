...
export function SellerDashboard() {
  ...
  const handleFoodBagSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error('Food bag creation/update error:', error)
      addToast({
        type: 'error',
  ...
  const handleOrderStatusUpdate = async (orderId: number, status: Order['status']) => {
    ...
    const isCriticalChange = status === 'cancelled' || status === 'completed'
    ...
    const updateOrder = async () => {
      try {
        ...
        addToast({
          type: 'success',
          message: `Order ${status === 'cancelled' ? 'cancelled' : status} successfully`
        })
        await loadOrders()
      } catch (error) {
    ...
    if (isCriticalChange) {
      const confirmationConfig = {
        title: status === 'cancelled' ? 'Cancel Order' : 'Complete Order',
        description: status === 'cancelled' 
          ? 'Are you sure you want to cancel this order? This action cannot be undone.'
          : 'Confirm that this order has been completed and picked up by the customer.',
        variant: (status === 'cancelled' ? 'destructive' : 'default') as 'destructive' | 'default',
        confirmText: status === 'cancelled' ? 'Cancel Order' : 'Mark as Completed',
      ...
      <div className="space-y-6 p-6">
        ...
        {!store ? (
          ...
          <TabsList className="grid w-full grid-cols-3 lg:grid-cols-5">
            ...
            </TabsContent>            {/* Inventory Tab */}            <TabsContent value="inventory" className="space-y-6">
              <Card className="glass-card border-border/30">
                          ...
                          <CardContent className="p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                              <div>
                                ...
                                <p className="font-medium">{order.food_bag?.title || 'Unknown'}</p>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <p className="font-medium">{formatIDR(order.total_price)}</p>
                              </div>
                            </div>
                            ...
                            {order.pickup_code && (
                              <div className="mb-4">
                                ...
                                <p className="font-mono text-lg font-bold">{order.pickup_code}</p>
                              </div>
                            )}
              ...
              <Card className="glass-card border-border/30">
                ...
                <CardContent>
                  {foodBags.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">                      {foodBags.map((foodBag) => (
              ...
              <Card className="glass-card border-border/30">
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <Label className="text-sm font-medium">Layout Style</Label>                      <Select value={customization.layout} onValueChange={(value: 'grid' | 'list' | 'carousel') => 
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Show Store Rating</Label>                        <Switch
                      ...
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Show Customer Reviews</Label>                        <Switch
                      ...
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Auto-approve Orders</Label>                        <Switch
                      ...
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Enable Notifications</Label>                        <Switch
