...
export function StoreDiscoveryPage() {
  ...
  const handleLocationSelect = (selectedLocation: { name: string; lat: number; lng: number }) => {
    ...
    // Always reset searchQuery and selectedCategory to default when location changes
    setSearchQuery('')
    setSelectedCategory('all')
    searchStores(selectedLocation.lat, selectedLocation.lng, 'all', '')
  }
  ...
  const handleCategoryFilter = (category: string) => {
    ...
    // Always reset searchQuery when changing category
    setSearchQuery('')
    if (userLocation) {
      searchStores(userLocation.lat, userLocation.lng, category, '')
    }
  }
  ...
  return (
    <AuthenticatedLayout showNavbar={true}>
                              ...
                              <div className="text-right">
                                <div className="flex items-center space-x-1">
                                  <span className="text-xs text-muted-foreground line-through">
                                    {formatIDR(bag.original_price)}
                                  </span>
                                  <span className="font-bold text-primary">
                                    {formatIDR(bag.discounted_price)}
                                  </span>
                                </div>
